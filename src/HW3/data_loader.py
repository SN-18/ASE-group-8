...
class DATA:
    ...
    def around(self, row1, rows=None, cols=None):
        ...
        temp = sort_co(map_co(rows or self.rows, fun), lt("dist"))
    ...
    def half(self, rows = None, cols = None, above = None):
        ...
        def project(row):
            d = {'row': row, 'dist': cosine(dist(row, A), dist(row, B), c)}
            return d
        ...
        B = temp[(the['Far'] * len(rows)) // 1]['row']
        # print("printing B in data.half",B)
        c = dist(A, B)
        # print("printing c in data.half",c)
        left, right = {}, {}
        sample = sort_co(map_co(rows, project), lt("dist"))
    ...
    def cluster(self, rows=None, min=None, cols=None, above=None):
        ...
        # print("printing self.cols.x",self.cols.x)
